...
// .wrangler/tmp/pages-98zQpB/functionsWorker-0.026466035272494692.mjs
...
var onRequestPost = /* @__PURE__ */ __name2(async (c) => {
  const { env, params } = c;
...
var onRequestGet = /* @__PURE__ */ __name2(async (c) => {
  const { env, params } = c;
  const auth = requireAuth(c, env);
  const user = await auth.getUser();
  if (!user) return new Response("Unauthorized", { status: 401 });
...
var onRequestPatch = /* @__PURE__ */ __name2(async (c) => {
...
var onRequestDelete = /* @__PURE__ */ __name2(async (c) => {
  const { env, params } = c;
  const auth = requireAuth(c, env);
  const user = await auth.getUser();
  if (!user) return new Response("Unauthorized", { status: 401 });
...
var onRequestPost2 = /* @__PURE__ */ __name2(async (c) => {
  const { env, request } = c;
  const body = await readJson(request);
...
var onRequestPost3 = /* @__PURE__ */ __name2(async (c) => {
  return json({ ok: true }, { headers: { "Set-Cookie": clearCookie("f127") } });
}, "onRequestPost");
var onRequestPost4 = /* @__PURE__ */ __name2(async (c) => {
  const { env, request } = c;
  const body = await readJson(request);
  if (!body) return bad("Invalid JSON");
  const username = sanitizeUsername(body.username);
...
var onRequestGet2 = /* @__PURE__ */ __name2(async (c) => {
  const { env } = c;
  try {
    const auth = requireAuth(c, env);
    const user = await auth.getUser();
...
var onRequestGet3 = /* @__PURE__ */ __name2(async (c) => {
  const { env } = c;
  const auth = requireAuth(c, env);
  const user = await auth.getUser();
  if (!user) return new Response("Unauthorized", { status: 401 });
...
var onRequestPost5 = /* @__PURE__ */ __name2(async (c) => {
  const { env, request } = c;
  const auth = requireAuth(c, env);
  const user = await auth.getUser();
  if (!user) return new Response("Unauthorized", { status: 401 });
...
var onRequestPost6 = /* @__PURE__ */ __name2(async (c) => {
  const { env } = c;
  const auth = requireAuth(c, env);
  const user = await auth.getUser();
  if (!user) return new Response("Unauthorized", { status: 401 });
...
var onRequestGet4 = /* @__PURE__ */ __name2(async (c) => {
  const { env } = c;
...
// .wrangler/tmp/bundle-NLG1Sa/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-NLG1Sa/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.026466035272494692.js.map

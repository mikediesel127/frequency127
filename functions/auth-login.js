...
export async function onRequest(context) {
  ...
  const valid = await verifyPass(u.salt, passcode, u.passhash);
  if (!valid) return json(401, { ok:false, error:'Invalid' });

  const token = await signJWT({ uid: u.id, t: Date.now() }, env.JWT_SECRET || 'dev');
  return new Response(JSON.stringify({ ok:true }), {
    status: 200,
    headers: { 'content-type': 'application/json', ...setAuthCookie(token) }
  });
}

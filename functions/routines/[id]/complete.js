...
export async function onRequest(context) {
  ...
  const exists = await q1(
    env,
    'SELECT 1 FROM routine_completions WHERE user_id=? AND routine_id=? AND day_key=?',
    user.id, id, key
  );
  if (exists) return json(200, { ok:true, already:true });

  await env.DB.prepare(
    'INSERT INTO routine_completions (user_id, routine_id, day_key, created_at) VALUES (?,?,?,?)'
